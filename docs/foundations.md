# Геометрические инварианты Lab Icons
...
ошибку можно было выявить при проверке, а решение — воспроизвести.
...
Шрифтовой файл не выбран источником модели: контракт проекта отдельно задаёт
роли частей, опорные точки движения, декораторы и календарную композицию.
Шрифтовой адаптер должен сохранить необходимые связи и построить совместимые
контуры; само наличие формата OpenType эту задачу не решает.
...
оптического равенства; соотношение круговой и квадратной keyline в
[сетке](../semantics/grid.json) является отдельным продуктовым решением. Оно сохраняется только пока corpus/raster review
...
просветы проверяются на размерах и DPR, объявленных соответствующим испытанием.
...
`glyphCapabilities().motion` различает состояния:
...
Смысловой жест управляет заявленными частями. Возможные направления развития
не являются возможностями текущего пакета: например, вращение всего земного
шара не доказывает независимое движение его сетки и поверхности. Общие
opacity/scale не заменяют такой контракт.
...
Жест `time.advance` управляет минутной и часовой стрелками. Диапазон, дорожки
и опорные точки принадлежат контракту жеста; испытание топологии проверяет
объявленное конечное множество значений прогресса, размеров и растровых фаз.
Этот результат не является доказательством для всех вещественных значений
прогресса и не означает наличия Lottie/SF Symbols файла.
...
`opsz` в `axisContracts` описывает общий диапазон. Ядра рецептов и варианты
используют этот источник, но возможность принадлежит конкретному варианту:
она появляется только при наличии `opsz` в его `supportedAxes` и прохождении
соответствующего испытания. Актуальный набор сообщает `glyphCapabilities()`.
Остальные варианты не получают скрытую подмену оси масштабированием SVG.
...
Это остаётся конечным испытанием, не аналитической теоремой для всех
...
- ошибка бинарной занятости на размерах из `DEFAULT_OBSERVATORY_RASTER_SIZES`;
...
Нижняя граница разрешаемой детали определяется бюджетом сетки и требуемым
числом отсчётов на деталь в [измерителе](../scripts/lib/quality-metrics.js):
`DEFAULT_TOPOLOGY_MAX_GRID_SIDE` и `DEFAULT_TOPOLOGY_PIXELS_PER_FEATURE`.
Для ширины канвы `W`, предела сетки `N` и `k` отсчётов линейная граница равна
`k·W/N`, граница площади — `(k·W/N)²`. Это часть контракта измерения, а не
произвольный численный epsilon. Нулевые петли экспортера и контуры ниже
объявленного пола не участвуют в топологической сигнатуре, но исходные
счётчики остаются в отчёте. Изменение бюджета или числа отсчётов требует
перепроверки контрактных свидетельств.
...
Отклонение выше `acceptance.autoDeviationPct` из
[политики качества](../semantics/quality-thresholds.json) требует конкретного
геометрического аргумента. Автоматически
...
## Источники и область их применения

Источники ниже описывают соответствующие платформенные модели. Они не
утверждают конкретные параметры Lab Icons: проектные пороги и выбранные
пропорции остаются политикой этого проекта.
