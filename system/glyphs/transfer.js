 ...
 */
export const REFRESH_CY = 13.5;
/**
 * НАРУЖНАЯ КРОМКА КОЛЬЦА — не подгонка, а та же линия, на которой стоит весь
 * класс «в круге»: keyR − ring = 9.5. Замер Outline даёт 9.515.
 *
 * В Filled рука вывела кольцо на 9.700, то есть на 0.185 ЗА эту линию и на
 * 0.14 за живую область (низ чернил 23.14 против 23). Это не следствие пера:
 * кап вырос на 0.3, а кромка — лишь на 0.185, значит скелет ещё и поджался.
 * Никакого закона, связывающего два начертания, здесь нет — есть два замера,
 * как у Regular и Bold шрифта, у которых метрики свои. Держать одну линию
 * стоило бы 12.96% на кольце, несущем 78% чернил; система следует замеру.
 *
 * Цена компромисса измерена: единый радиус-минимакс 8.56 давал 5.55% и 5.77%,
 * то есть промах в оба начертания сразу. Два замера дают 1.94% и 3.17%.
 */
export const REFRESH_OUTER = (t) => (t.fill >= 0.5 ? 9.7 : t.keyR - t.stroke.ring);
export const REFRESH_R = (t) => REFRESH_OUTER(t) - t.cap.glyph;
...
defineGlyph('refresh', {
  ...
  law:
    ...
    'выйти за живую область, поэтому наружная кромка Filled взята замером 9.70',
  argument:
    'остаточные 3.17% в Filled — цена того, что рука НЕ держит в жирном начертании ту линию, ' +
    'на которой стоит весь класс «в круге». Замеры: Outline наружная кромка 9.515 при законе ' +
    'keyR − ring = 9.5 (промах 0.015), Filled — 9.700, то есть 0.185 за линию и 0.14 за живую ' +
    'область (низ чернил 23.14 против 23). Пером это не объясняется: кап вырос на 0.3, а кромка ' +
    'лишь на 0.185, значит скелет ещё и поджался с 8.615 до 8.5. Формулы, связывающей два ' +
    'начертания, в корпусе нет — есть два замера, как у Regular и Bold шрифта. Цена альтернатив ' +
    'измерена: держать одну линию — 1.94% и 12.96%; единый радиус-минимакс 8.56 — 5.55% и 5.77%, ' +
    'то есть промах в оба начертания сразу. Два замера дают 1.94% и 3.17%, и остаток лежит уже ' +
    'не в кольце, а в голове: пятна расхождения площадью 1.46 и 0.89 стоят на плечах головы ' +
    '(11.6…16.5 × 5.6…9.4 и 11.7…16.4 × 0.8…4.4), кольцо совпало.',
  outline: (t) => {
    const w = t.stroke.glyph;
    const c = [t.cx, REFRESH_CY];
    const R = typeof REFRESH_R === 'function' ? REFRESH_R(t) : REFRESH_R;
    const p = bandRound(c, R, 0, rad(270), w);
    const axis = REFRESH_CY - R;
...
/**
 * ОСЬ ГОЛОВЫ — по замеру терминалов плеч, и у двух начертаний он свой.
 * Outline: верхняя голова 10.493 и 10.525 (сама рука развела свои же два плеча
 * на 0.032), нижняя 13.567 — точечная симметрия требовала бы 10.433, то есть
 * рука разошлась с собой на 0.076. Взято среднее 10.51.
 * Filled: верхняя 10.570, нижняя 13.462/13.470 → 10.534 по симметрии. Взято 10.57.
 */
export const SYNC_STEM = (t) => (t.fill >= 0.5 ? 10.57 : 10.51);

const syncArm = (t) => {
  const w = t.stroke.glyph;
  const R = ARC_INK - t.cap.glyph;
  const c = [t.cx - SYNC_DX, t.cy];
  const apex = [c[0] - R, SYNC_STEM(t) - HEAD_HALF];
...
defineGlyph('sync', {
  ...
  argument:
    'остаточные 3.65% / 3.19% — посадка, не форма. Вердикт registration: медиана смещения контура ' +
    '0.052 ед = 2.9% пера, при пороге вердикта 0.06. Площадная метрика на штрихе нелинейна ' +
    '(1−IoU ≈ 2δ/(2w−δ)): при пере 1.8 смещение 0.027 ед уже стоит 3%, то есть весь остаток ' +
    'умещается в две сотых единицы. Замер терминалов подтверждает: мои плечи стоят на ' +
    '(7.740, 10.550) и (1.900, 10.550), рука — на (7.773, 10.493) и (1.910, 10.525), расхождение ' +
    '0.03…0.06. И сама рука с собой не сходится: её ДВА плеча одной головы разведены на 0.032, ' +
    'а нижняя голова относительно верхней нарушает точечную симметрию на 0.076 — вдвое больше ' +
    'того, в чём система обвиняется. Пятна расхождения: 0.31 ед² толщиной 0.31 на левой кромке ' +
    'дуги и 0.16 ед² толщиной 0.38 на плече головы — полоски, а не детали',
  outline: (t) => {
    const a = syncArm(t);
...
defineGlyph('repeat', {
  ...
  argument:
    'остаточные 3.85% / 3.16% — вердикт registration, медиана смещения контура 0.038 / 0.045 ед = ' +
    '2.1% / 1.9% пера при пороге 0.06. При пере 1.8 три процента стоят 0.027 ед, поэтому весь ' +
    'остаток лежит в четырёх сотых единицы. Локализация честная: два пятна по 0.3 ед² — на ' +
    'локте полосы (10.3…11.4 × 14.1…15.7, толщина 1.19) и на голове (14.1…16.5 × 2.4…4.8, ' +
    'толщина 2.38). То есть голова посажена не идеально, и это НЕ объяснено законом — замер ' +
    'терминалов руки даёт плечи на x = 13.510 при y 3.033 и 8.955, полуразмах 2.961 против ' +
    'токена 2.92; разница в 0.04 и есть остаток. Дуга при этом совпала точно: наружная кромка ' +
    'ARC_INK = 7.8 подтверждена замером 7.800 в Outline и 7.900 в Filled',
  outline: (t) => {
    const a = repeatArm(t);
