 ...
 * Outline и 0.95 в Filled, подгон 1.585 и 1.91).
 */
export const RADIO_PEN = Object.freeze({ outline: 1.585, filled: 1.91 });
...
defineGlyph('headphone', {
  ...
  argument:
    'ЧЕСТНО: единственное имя семьи, которое не сошлось. Сошёлся ОБОД — и это проверяемо: ' +
    'внешняя кромка обода лежит на окружности 11.0 = Rkey, внутренняя на 9.2 = Rkey − перо, ' +
    'два прямых замера по d, без единого подобранного числа. Не сошлась ЧАШКА, и она — ' +
    'рисунок, а не конструкция. Её внешняя кромка трижды меняет кривизну: у острия дуга ' +
    'радиуса 2.92 с центром (19.01, 14.95), в середине 8.35 с центром (13.55, 15.32), внизу ' +
    'около 6.1 — три разных центра на одном контуре длиной 20 ед. Ни один односоставный ' +
    'примитив такого не даёт. Выпуклая оболочка двух кругов держит остриё (11.77 против ' +
    '11.68) и низ (22.89 против 23.01), но недобирает вынос наружу (21.48 против 21.89) и ' +
    'переливает внутрь у нижней кромки: два кластера по 6 ед² при чернилах 158 ед². Отсюда ' +
    '21.5% площади (Outline) и 14.3% (Filled) при медиане смещения контура 0.100 и 0.079 ед ' +
    '— то есть контур в основной массе стоит на месте, а расходится ровно на двух дугах ' +
    'чашки. Объявлен каркас: обод, ось чашки 60.35° и её габарит. Дотянуть чашку до ' +
    'совпадения можно только перечислив её узлы, то есть отказавшись от закона.',
