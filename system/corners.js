...
export function spectrum(subs) {
  ...
  for (const sub of subs) {
    ...
    /**
     * ЯКОРЬ РАЗРЕЗА — СЕРЕДИНА САМОГО ДЛИННОГО ХОЛОДНОГО ПЛАТО, а не первый
     * минимум.
     *
     * Разрез нужен затем, чтобы ни одна угловая область не разрывалась границей
     * массива. Но выбор «первый индекс с наименьшей суммой k + импульс» НЕ
     * ИНВАРИАНТЕН К НАЧАЛУ ОБХОДА: на прямом ребре кривизна ровно ноль у сотен
     * выборок подряд, минимум достигается всем плато, и strict `<` берёт тот его
     * конец, который встретился первым.
     *
     * Дефект пойман на заведомо СИММЕТРИЧНОЙ фигуре: левая чашка наушников есть
     * зеркало правой, sampleSub у них совпадает до числа (2084 выборки, 758
     * горячих, kmax 0.588), а спектр читал справа 2 узла, слева 3 — потому что
     * длинное прямое ребро у правой чашки идёт ПЕРВЫМ ребром подпути, у левой
     * последним, и якорь садился на противоположные концы одного и того же
     * ребра. Прибор, зависящий от того, с какого узла автор начал писать путь,
     * измеряет запись, а не фигуру.
     *
     * Середина плато инвариантна и к началу обхода, и к зеркалу: плато
     * отображается в плато, его середина — в середину. И стоит она дальше всего
     * от любого угла, что якорю и требуется.
     */
    const coldness = (i) => at(i).k + at(i).imp;
    let cold = 0;
    const flat = new Array(n);
    for (let i = 0; i < n; i++) flat[i] = coldness(i) <= K_FLAT;
    const runStarts = [];
    for (let i = 0; i < n; i++) if (flat[i] && !flat[(i - 1 + n) % n]) runStarts.push(i);
    if (runStarts.length) {
      let bestLen = -1;
      for (const s of runStarts) {
        let len = 0;
        while (len < n && flat[(s + len) % n]) len++;
        if (len > bestLen) {
          bestLen = len;
          cold = (s + (len >> 1)) % n;
        }
      ...
      for (let i = 1; i < n; i++) if (coldness(i) < coldness(cold)) cold = i;
    }
    const gap = (a, b) => {
    ...
    /**
     * Области не имеют права накладываться: иначе один и тот же поворот попадёт
     * в два угла, и сумма по контуру уедет за положенные 360°.
     *
     * РАЗВОД СЕРЕДИНОЙ ПРОВЕРЕН И ОТВЕРГНУТ. Порядок обхода тут виден: области
     * растут по очереди от якоря, и кто первый — тот и занял спорные выборки.
     * Казалось, что честнее дать каждой вырасти свободно и поделить пересечение
     * пополам — середина симметрична по определению. Замер этого не подтвердил:
     * несимметрично читаемых вариантов осталось столько же (5 из 64 с
     * симметричными чернилами), а расходящихся по корпусу стало 56 вместо 53.
     * Значит остаточная асимметрия живёт не здесь, и менять правило разводки без
     * причины — портить корпус ради догадки.
     */
    const regions = [];
