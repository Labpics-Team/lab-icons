...
export function roundedPolygon(points, radius, smoothing = 0) {
   ...
   * профиль входа, и когда разбега td·(1+ζ) не хватает, честный ответ —
   * поставить в этой вершине голую галтель, а не поджать измеренный радиус.
   *
   ...
   */
  const roomForEase = new Array(n).fill(true);
  if (smoothing > 1e-6) {
    for (let i = 0; i < n; i++) {
      const j = (i + 1) % n;
      if (tanDist[i] * scale[i] * (1 + smoothing) + tanDist[j] * scale[j] * (1 + smoothing) > len[i] + 1e-9) {
        roomForEase[i] = false;
        roomForEase[j] = false;
      }
    }
  ...
  for (let i = 0; i < n; i++) {
    ...
    const s = smoothing;
    if (s > 1e-6 && scale[i] > 0.999 && roomForEase[i]) {
