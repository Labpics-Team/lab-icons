...
export const TOKENS = Object.freeze({
  ...
  corner: Object.freeze({
    ...
    smoothing: 0.6,
    /**
     * ФАСКА ВНУТРЕННЕГО ЛОКТЯ штрихового сустава — в ДОЛЯХ ПЕРА, не в единицах
     * канвы: это свойство инструмента, оно обязано ехать за весом. Замеры руки
     * дают 0.23…0.39 при пере 1.8, то есть 0.13…0.22 пера; шестая доля стоит
     * посередине. Голое остриё здесь режет просвет между плечами и первым
     * забивается на мелком кегле.
     */
    joint: 1 / 6,
    /**
     * ГАЛТЕЛЬ НАРУЖНОГО ЛОКТЯ — тоже в долях пера. Круглый сустав дал бы 0.5
     * (перо/2 вокруг вершины скелета), но рука пишет на локте шеврона
     * буквальное `A1.8 1.8` при пере 1.8, то есть РОВНО перо. По 32 локтям
     * корпуса R/перо = 0.85…1.00 при медиане 0.93; единица — модальное
     * значение, которое рука выписывает точно.
     */
    elbow: 1,
  }),

  /**
   * КЛИН — треугольный указатель воспроизведения.
   *
   * Пропорции сняты со всех десяти оригиналов семейства play и оказались
   * ЕДИНЫМИ: у каждого клина поворот в вершинах 105…120°, то есть треугольник
   * один и тот же, меняется только высота. Радиус скругления идёт за высотой
   * (0.18 H) и ОДИНАКОВ во всех трёх вершинах — острия рука не выделяет.
   *
   * Числа: B/H = 1.266 (play-circle), 1.244 (двойной), 1.287 (со стенкой);
   * R/H = 0.170 (play), 0.174 (play-forward), 0.195 (play-circle).
   *
   * До этого в системе жили ЧЕТЫРЕ несогласованных подгонки одной фигуры —
   * 1.5/1.15 у play-circle, 0.76/0.36 у клина со стенкой и НОЛЬ у двойного.
   * Нулевой радиус пережимал штрих в точку там, где рука держит 1.41.
   */
  wedge: Object.freeze({
    /**
     * ДВЕ ПРОПОРЦИИ, и обе точные.
     *
     * free — РАВНОСТОРОННИЙ треугольник. Не подгонка: угол при острие 60.01° у
     * play/outline и 59.98° у play/filled, то есть основание/высота = 2/√3
     * ровно. Свободный клин у руки — правильная фигура, и константа здесь
     * ВЫВОДИТСЯ, а не измеряется.
     *
     * inset — клин внутри круга и в перемотке: угол при острие 64.60…64.82°
     * (play-circle 64.76/64.75, play-back 64.82/64.60), основание/высота 1.269.
     * Стабильно во всех членах и стабильно ОТЛИЧНО от равностороннего: мелкий
     * клин рука делает чуть коренастее.
     */
    aspectFree: 2 / Math.sqrt(3),
    aspectInset: 1.269,
    /**
     * Радиус скругления вершины / высота — ОДИН на все три вершины и один на
     * оба типа клина. Замеры: play 3.30/21.45 = 0.1538, play-circle
     * 1.437/9.280 = 0.1549, play-back 2.095/13.00 = 0.1612.
     */
    corner: 0.155,
    /** доля высоты позади оптического центра (центроид треугольника — 1/3) */
    anchor: 0.358,
  }),
...
export function resolve(opt = {}) {
  ...
  return {
    ...
    corner: {
      ...
      smoothing: crnr,
      joint: TOKENS.corner.joint * rond,
      elbow: TOKENS.corner.elbow * rond,
    },
    // Ось rond управляет и скруглением клина: клиент, которому нужен «мягкий»
    // набор, получает его целиком, а не по одной фигуре.
    wedge: { ...TOKENS.wedge, corner: TOKENS.wedge.corner * rond, aspect: TOKENS.wedge.aspectInset },
    fill,
