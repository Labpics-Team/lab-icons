...
export class Path {
  ...
  /**
   * ЗЕРКАЛО. Обход ВОССТАНАВЛИВАЕТСЯ, и это не деталь реализации, а защита от
   * целого класса дефектов.
   *
   * Отражение — преобразование с отрицательным определителем, и оно
   * переворачивает намотку каждого подпути. Пока зеркалится путь ЦЕЛИКОМ, это
   * незаметно: внешний контур и его дырка меняют знак вместе, и nonzero даёт
   * ту же заливку. Но стоит положить зеркальную часть в один путь с
   * НЕзеркальной — и в их перекрытии намотка складывается в +1 − 1 = 0, то есть
   * ЧЕРНИЛА ГАСНУТ.
   *
   * Именно это и произошло у наушников: обод не зеркалится, левая чашка
   * зеркалится, и в месте, где чашка накрывает обод, силуэт распадался на три
   * куска с двумя трещинами — причём ВСЕ дефекты были слева и ни одного справа.
   * Площадь такое почти не двигает, детектор топологии показал сразу.
   *
   * Восстановление обхода безопасно и для целого пути (заливка не меняется), и
   * обязательно для части. Поэтому оно здесь по умолчанию, а не на совести
   * вызывающего.
   */
  mirrorX(ax) {
    return this.transform({ a: -1, b: 0, c: 0, d: 1, e: 2 * ax, f: 0 }).reverse();
  }

  /** Зеркало относительно горизонтали y = ay; обход восстанавливается. */
  mirrorY(ay) {
    return this.transform({ a: 1, b: 0, c: 0, d: -1, e: 0, f: 2 * ay }).reverse();
  }
