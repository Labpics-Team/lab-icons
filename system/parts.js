 ...
 */
/**
 * ВЫНОС ЧЕРНИЛ ОСТРИЯ ЗА СКЕЛЕТНУЮ ВЕРШИНУ — вывод, не замер.
 *
 * У вершины с полураствором `open` наружная кромка штриха отходит на w/2, и две
 * такие кромки сходятся в миterной точке на (w/2)/sin(open) от скелета. Локоть
 * радиуса R = w·elbow срезает миter: его центр стоит на (w/2 − R)/sin(open), а
 * чернила доходят до центра плюс R. Отсюда
 *
 *     вынос = (w/2 − R)/sin(open) + R,
 *
 * и при R = w, open = 45° это 0.2929·w — то самое число 0.527 при пере 1.8.
 *
 * Нужно оно затем, что у стрелки закреплены ЧЕРНИЛА кончика (замер: 19.00 в
 * трёх из четырёх направлений в ОБОИХ начертаниях), а вынос растёт вместе с
 * пером. Считать вершину как «чернила минус кап» — приближение: при Regular оно
 * ошибается на 0.05, при Bold уже на 0.08, и кончик Filled уезжал назад там,
 * где у руки он стоит на месте.
 *
 * ЧТО ЭТА ФОРМУЛА НЕ УЧИТЫВАЕТ: локоть идёт с ζ-сглаживанием, и оно поджимает
 * кончик. Замер по построенным путям: фактический вынос 0.470 при пере 1.8 и
 * 0.640 при 2.4, то есть 0.261·w и 0.267·w против идеальных 0.293·w — голая
 * галтель дала бы на 10% больше. Коэффициент 0.2929 оставлен как ВЫВОД
 * геометрии, а разница поглощается калибровкой самой константы чернил
 * (ARROW_INK_TIP снят прогоном по всей семье, минимум среднего на 7.00).
 */
export const headTipOver = (w, open, elbow = 1) => {
  const R = w * elbow;
  return (w / 2 - R) / Math.sin(rad(open)) + R;
};
...
export function arrow(t, o = {}) {
  ...
  /**
   * ХВОСТ НАЧИНАЕТСЯ НЕ В ВЕРШИНЕ, А В КАПЕ ОТ НЕЁ.
   *
   * Круглый терминал хвоста — полукруг радиуса перо/2 вокруг своей начальной
   * точки. Поставленный ровно в вершину, он поднимается над ней на 0.900, тогда
   * как галтель острия головы поднимается лишь на r·(1/cos(Θ/2) − 1) вычтенное
   * из миterного выноса — при пере 1.8 и растворе 45° это 0.527. Терминал
   * ВЫЛЕЗАЛ из головы на 0.373 и давал узкий купол с двумя плечами там, где у
   * руки один широкий свод. Спектр углов нашёл этот дефект в пяти вариантах
   * корпуса при отклонении 0.3…1.3% — площадь его не видит вовсе.
   *
   * Отступ ровно в кап прячет терминал под чернила головы и ничего не меняет в
   * силуэте: этот кусок хвоста и так накрыт головой.
   */
  if (tail > 0) {
    const from = [apex[0], apex[1] - w / 2];
    p.add(strokeSegment(from, [apex[0], apex[1] - tail], w));
  }
  const deg = DIR[o.dir ?? 'down'] ?? 0;
