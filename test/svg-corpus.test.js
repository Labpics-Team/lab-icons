 ...
 *
 * Характеризационные тесты: каждый инвариант ЭМПИРИЧЕСКИ проверен на всех 476
 ...
 *   7. Числа в d: без научной нотации. Авторская дробная точность сохраняется;
 *      бюджет квантования относится к build artifact, а не к source master.
  ...
  for (const { dir } of SETS) {
    it(`3–7: корень/атрибуты/пары/цвета/числа — ${dir}`, () => {
      ...
      for (const f of corpus[dir]) {
        ...
        if (!f.src.trimStart().startsWith('<svg')) violations.push(`${tag}: мусор до корневого <svg>`);
