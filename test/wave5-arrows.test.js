...
const FLOORS = {
  ...
  // swap-horizontal: полы пере-зафиксированы 2026-07-08 после закона смежности
  // (торцевой кап палочки касается вершины шеврона, щель в раскрыве → 0):
  // чернила закона лежат там, где рука держала пустой раскрыв — падение IoU
  // осознанное (lawOverHand + correctionReason в декларации), замер .9378/.9198
  'swap-horizontal': { outline: 0.9358, filled: 0.9178 },
  ...
  for (const [name, floors] of Object.entries(FLOORS)) {
    it(`${name}: IoU против рук не ниже замороженного пола`, () => {
      ...
      for (const [variant, floor] of Object.entries(floors)) {
        ...
        // swap-horizontal: закон смежности сознательно уводит генерат ниже
        // корпусного hand-порога (перекрытие стыка поверх пустого раскрыва
        // руки) — исключение задокументировано в декларации (lawOverHand),
        // идентичность держит пер-иконный пол characterization выше
        if (name !== 'swap-horizontal') {
  ...
  for (const name of Object.keys(FLOORS)) {
    it(`${name}: ТОЧНОЕ равенство EO≡NZ по всем вариантам (шаг 0.12)`, () => {
      ...
      for (const [variant, d] of Object.entries(built)) {
        ...
        // swap-horizontal (оба варианта): стык head+shaft ПЕРЕКРЫВАЕТСЯ по закону
        // смежности (scripts/check-adjacency.js) — торцевой кап палочки касается
        // вершины шеврона (зазор→0), иначе рендер даёт видимо отрезанную часть
        // при 98% IoU. EO≠NZ в линзе перекрытия ОЖИДАЕМ (одноцветные чернила,
        // nonzero-рендер браузера идентичен). Связность стыка гарантирует
        // check-adjacency (HARD, промоушен), а не этот EO≡NZ-ассерт (тот
        // кодировал старое «стык без перекрытия» — корень дыры).
        if (name === 'swap-horizontal') continue;
