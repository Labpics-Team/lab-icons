 ...
 * composite stroke-path (head 3т + shaft 2т, стык БЕЗ перекрытия: тангенс-
 * солвер, у swap-horizontal — сокет-встык по клину шеврона (socket, 2026-07-09)
 * — отсюда адресный EO≡NZ-ассерт).
 *
...
const FLOORS = {
  ...
  // swap-horizontal: полы пере-зафиксированы 2026-07-09 после стыка
  // сокетом-встык (genStrokePath socket по клину шеврона, класс time):
  // сокет снял лишние чернила перекрытия из раскрыва — замер .9870/.9872
  // (было .9378/.9198 при перекрытии, резавшем полумесяц под evenodd)
  'swap-horizontal': { outline: 0.985, filled: 0.9852 },
  ...
  for (const [name, floors] of Object.entries(FLOORS)) {
    it(`${name}: IoU против рук не ниже замороженного пола`, () => {
      ...
      for (const [variant, floor] of Object.entries(floors)) {
        ...
        // корпусный hand-порог держит ВСЯ семья: исключение swap-horizontal
        // снято 2026-07-09 — сокет-встык вернул IoU выше 0.95 (.987/.9872)
  ...
  for (const name of Object.keys(FLOORS)) {
    it(`${name}: ТОЧНОЕ равенство EO≡NZ по всем вариантам (шаг 0.12)`, () => {
      ...
      for (const [variant, d] of Object.entries(built)) {
        ...
        // swap-horizontal БЕЗ исключения (2026-07-09): перекрытие стыка давало
        // белый полумесяц в линзе под evenodd (скриншот владельца) — стык
        // переведён на сокет-встык (genStrokePath socket, класс time): касание
        // без перекрытия и без щели, EO≡NZ точное; связность держит
        // check-adjacency (HARD, промоушен).
