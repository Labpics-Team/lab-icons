...
describe('качество контура', () => {
  ...
  it('места нет — поджимается СГЛАЖИВАНИЕ, а радиус остаётся', () => {
    // Разбег сглаживания td·(1+ζ) съедает ребро сильнее, чем само касательное
    // расстояние. Раньше это не проверялось, и на большом радиусе разбег вылезал
    // за соседнюю вершину: контур складывался сам на себя (внутренний контур
    // дротика navigate распадался на семь кусков). Радиус при нехватке места
    // трогать нельзя — он замер; поджимается профиль входа, и ровно настолько,
    // насколько нужно: выключать его целиком стоило 40 узлов корпуса.
    for (const r of [2, 4, 6, 7]) {
      const es = edgesOfPath(S.roundedRect(12, 12, 16, 16, r, 0.6))[0].edges;
      const arcs = es.filter((e) => e.kind === 'arc');
      expect(arcs, `r=${r}: четыре угла`).toHaveLength(4);
      for (const a of arcs) expect(a.r, `r=${r}: радиус сохранён`).toBeCloseTo(r, 6);
      // и контур не сложился: полный поворот замкнутой кривой ровно 360°
      let total = 0;
      for (const q of sampleSub(edgesOfPath(S.roundedRect(12, 12, 16, 16, r, 0.6))[0])) total += q.simp + q.dturn;
      expect(Math.abs(Math.abs(total) - 360), `r=${r}: поворот ${total.toFixed(1)}°`).toBeLessThan(1.5);
    }
  });
  ...
  it('настоящий зазор от шва растеризатора отличается масштабированием', () => {
    // Шов заливки там, где две почти совпавшие кромки сходятся под острым углом,
    // всегда ровно два пикселя: 0.250 при ss=8, 0.126 при 16, 0.100 при 20 —
    // то есть 2/ss, и с разрешением он тает. Настоящий зазор держит ширину в
    // единицах канвы. Порог по абсолютной ширине для различения НЕ годится:
    // доказанные трещины наушников мерились 0.2 ед при ss=10, ровно на том же
    // пределе, и любой порог спрятал бы настоящий дефект.
    const gap = 0.2;
    // щель НЕ доходит до краёв: тогда она настоящая дырка, а не разрез плиты
    const holed = cut(S.roundedRect(12, 12, 14, 10, 1), S.roundedRect(12, 12, gap, 6, gap / 2));
    const width = (ss) =>
      topo
        .topology(maskFromPath(holed, 24, ss, 0.004), { canvas: 24, ss })
        .holes.filter((h) => 2 * h.maxR < 0.5)
        .map((h) => 2 * h.maxR)
        .sort((a, b) => b - a)[0] ?? 0;
    const w8 = width(8);
    const w16 = width(16);
    expect(w8, 'зазор виден на грубом растре').toBeGreaterThan(0.1);
    expect(w16, 'и НЕ ужимается вдвое на мелком — значит он настоящий').toBeGreaterThan(w8 * 0.6);
  });
