...
describe('качество контура', () => {
  ...
  it('места нет — снимается СГЛАЖИВАНИЕ, а радиус остаётся', () => {
    ...
    // трогать нельзя — он замер; снимается профиль входа.
    const r = 6;
    const eased = edgesOfPath(S.roundedRect(12, 12, 16, 16, r, 0.6))[0].edges;
    expect(eased.some((e) => e.kind === 'cubic'), 'при r=6 на стороне 16 разбега нет').toBe(false);
    const arcs = eased.filter((e) => e.kind === 'arc');
    expect(arcs).toHaveLength(4);
    for (const a of arcs) expect(a.r, 'радиус сохранён').toBeCloseTo(r, 6);
  });
