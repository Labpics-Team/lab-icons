...
const LEGACY_EO_NZ = new Set([
  ...
  'reload/filled',
  'reload/outline',
  // swap-horizontal/outline: НЕ легаси-дефект, а НАМЕРЕННОЕ перекрытие стыка
  // палочка↔наконечник по закону смежности (scripts/check-adjacency.js). Без
  // перекрытия рендер даёт видимо отрезанную часть при 98% IoU — корень гейт-дыры.
  // EO≠NZ в линзе перекрытия одноцветных чернил безвреден (nonzero-рендер браузера
  // идентичен); связность стыка гарантирует check-adjacency. Правило «только
  // уменьшаться» относится к легаси-грязи, а не к этому осознанному стыку.
