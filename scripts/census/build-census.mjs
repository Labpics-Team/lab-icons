...
// --- аттрибуция dist/ir/index.js по маркерам esbuild ---
let bundle = null;
try {
  const src = readFileSync(join(ROOT, 'dist', 'ir', 'index.js'), 'utf8');
  const markers = [...src.matchAll(/^\/\/ ([^\n]+)$/gm)].map((m) => ({ path: m[1], idx: m.index }));
  const mods = markers.map((m, i) => ({
    path: m.path,
    bytes: (i + 1 < markers.length ? markers[i + 1].idx : src.length) - m.idx,
  }));
  mods.sort((a, b) => b.bytes - a.bytes);
  bundle = { totalBytes: src.length, modules: mods };
} catch {
  // dist ещё не собран — census без bundle-секции, verify соберёт до вызова
}
