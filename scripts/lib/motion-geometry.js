/**
 * scripts/lib/motion-geometry.js — математика гейтов моушна (BL-006, zero-dep).
 *
 * Модель ровно повторяет рантайм: слой = path с transform-box: view-box и
 * transform-origin в точке-якоре; кадры генерата — строки transform с
 * согласованной структурой функций (WAAPI интерполирует аргументы линейно
 * между кадрами — здесь та же интерполяция). Контур слоя — полилинии
 * суб-путей из точного парсера path-data (де Кастельжо/шаг по дуге).
 *
 * Экспорт:
 *   parseTransformString(s)          → [{fn, args}] в порядке следования
 *   transformPoint(p, fns, anchor)   → точка после transform вокруг якоря
 *   transformAt(part, tMs, staggerMs)→ функции transform части в момент t
 *                                      (delay + fill both + линейная интерполяция)
 *   samplePolylines(d, stepsPerSeg)  → полилинии суб-путей (для рёбер/чернил)
...
export function parseTransformString(s) {
  ...
  for (const m of s.matchAll(FN_RE)) {
    ...
    if (args.some((v) => !Number.isFinite(v))) {
      throw new Error(`motion-geometry: неконечный аргумент в transform "${s}"`);
    }
...
export function transformAt(part, tMs, staggerDelayMs) {
  ...
  if (fa.length !== fb.length) {
    throw new Error('motion-geometry: структура transform между кадрами не совпадает');
  }
  return fa.map((f, i) => ({
    fn: f.fn,
