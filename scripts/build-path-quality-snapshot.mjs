#!/usr/bin/env node
/**
 * scripts/build-path-quality-snapshot.mjs — воспроизводимое переснятие обоих
 * debt-baseline для check-path-quality:
 *   - semantics/path-quality-by-source.json (per-source ledger, closed world 444)
 *   - semantics/legacy-quality-snapshot.json (глобальный frozen set, только
 *     секция pathQuality; variantParity не трогаем)
 *
...
console.log(
  `build-path-quality-snapshot: ${allNames.length} файлов, ` +
    `${findings.length} находок (${major.length} major) — оба snapshot переснятия записаны`,
);
