// _decl-move.mjs — вставка декларации move в semantics/anatomy.json (Волна-7).
// Нетрекаемый скрипт, удалить до merge.
import { readFileSync, writeFileSync } from 'node:fs';

const raw = readFileSync('semantics/anatomy.json', 'utf8');
const a = JSON.parse(raw);
if (a.glyphs.move) throw new Error('move уже задекларирован');

const fitted = JSON.parse(readFileSync('_move-entry.json', 'utf8'));
const m = fitted.m;
a.glyphs.move = {
  archetype: 'composite',
...
console.log('move вставлен; fidelityToHand:', JSON.stringify(a.glyphs.move.fidelityToHand));
