...
## Токенная система `system/` — конструкция вместо рисунка

Слой, в котором иконка перестаёт быть файлом и становится **декларацией**:
закон словами плюс построитель из токенов. Файл `.svg` производен.

Задача слоя — убрать место, куда можно вписать выдуманное число. Радиусы,
внутренние кромки и габариты не задаются, а **выводятся**:

| вывод | формула | при канве 24 | замер руки |
|---|---|---|---|
| терминал | `кап = перо / 2` | 0.9 | `a.9 .9` во всём корпусе |
| внутренняя кромка | `rIn = rOut − перо` | 3.2 при rOut = 5 | square, mail, диск sun |
| скруглённый квадрат | `h = R + (Rkey − R)/√2` | габарит 18.485 | 18.48 (`square.svg`) |
| осевой keyline | снят с корпуса | 6 | minus 6.00 · plus 6.06 · close 6.01 |
| предел луча | `Rkey − кап − Rтела − зазор − кап` | 3.40 | sun 1.8 · sun-low 0 |

```
system/
  tokens.js         конституция: всё долями канвы, плюс оси вариативности
  core/             модель пути (дуга = центр + углы), пересечения, разность
  prim/             примитивы-формулы и обводка скелета в контур
  parts.js          части со смыслом и накладные классы (перечёркивание, бейдж)
  numerals.js       собственный округлый знак 0–9 — без шрифтовых зависимостей
  glyphs/           декларации
  metrics.js        сходимость: площадь + смещение контура
  build.js preview.js promote.js check-system.js
```

**Оси вариативности.** Иконка — функция от осей, как начертание вариативного
шрифта. Границы выведены, а не выбраны: `wght ∈ [0.6, 1.29]`, где низ — предел
различимости пера (кольцо не тоньше собственного терминала), а верх — предел
негативного пространства (канал между двумя Bold-штрихами не уже охранного
зазора). Локальные оси объявляет глиф: длина луча превращает `sun` в `sun-low`,
длина хвоста превращает стрелку в шеврон.

```ts
import { icon, svg, manifest } from '@labpics/icons/system'
icon('sun', { axes: { ray: 3.4 }, wght: 1.2 })   // строка d
svg('square', { variant: 'filled', crnr: 0 })    // готовый <svg>
manifest()                                        // токены, оси, законы глифов
```

**Как меряется сходимость.** Двумя мерами, и обе обязательны. Площадное
`1 − IoU` по чернилам — то, что видно на превью первой колонкой. И смещение
контура в единицах канвы, потому что площадная метрика на штрихах нелинейно
жестока: `1 − IoU ≈ 2δ/(2w − δ)`, то есть при пере 1.8 смещение 0.027 ед. уже
даёт 3% — меньше, чем разброс самой руки (у `chevron-down` два терминала одного
штриха стоят на 0.063 друг от друга). Подробно: [docs/system.md](docs/system.md).

```bash
pnpm system            # отчёт сходимости
pnpm system:preview    # docs/preview.html — оригинал | генерат | % | аргумент
node system/check-system.js   # гейт: расхождение > 3% без аргумента = красный
node system/promote.js        # что готово к переносу в поставку (сухой прогон)
```

`reference/` — замороженные оригиналы руки, извлечённые из первого коммита
каждого файла. Точка отсчёта сходимости; ничего, кроме чтения.
