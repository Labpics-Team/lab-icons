...
как одна из его проекций. Сейчас корпус мигрируется по семействам:
...
- drift/fidelity/topology/weight/adjacency gates проверяют материализованный
  результат;
- preview и immutable hand baseline развиваются в R0 Quality Observatory.
...
Подпуть `@labpics/icons/animate` уже доступен в артефакте, но остаётся
**экспериментальным** до стабилизации Glyph IR v2 и устойчивых `part.id`.
Расширение анимаций не должно диктовать структуру статической геометрии.
...
`@labpics/icons` — внешнее ядро LabUI. LabUI должен потреблять пакет через
публичный контракт и не копировать SVG или сборочный pipeline. Переход от
временного workspace-стаба к реальной зависимости выполняется отдельным
интеграционным PR с SSR, hydration, accessibility и tree-shaking fixtures.
